	...
	function showRandPosts(randList, randListUrl, Counter) {
		...
		query.find().then(function (result) {
			randList.each(function () {
				...
				if(r.length > 0) {
					$(this).append('<span class="badge">' + '<i class="fa fa-eye"></i> ' + r[0].get('time') + '</span>')
				} else {
					$(this).append('<span class="badge"><i class="fa fa-eye"></i> 0</span>')
				}
			})
		}).then(function (error) {
			console.log('Error:' + error.code + ' ' + error.message)
			...
			for(var i = 0; i < results.length; i++) {
				...
				hotPostsList.each(function () {
					$(this).append(`<li class="list-group-item">
                							...
                							<span class="badge"><i class="fa fa-eye"></i> ${time}</span>
            							</li>`)
				})
			}
		}).then(function (error) {
