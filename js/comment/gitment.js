var galTheme = {
	...
	renderHeader(state, instance) {
		...
		if(state.comments) {
			if(state.comments.length > 0) {
				$(container).append(`<i class="fa fa-comments"></i> ${state.comments.length}Comment`)
			} else {
	...
	renderComments(state, instance) {
		...
		if (state.comments) {
			for(let i = 0; i < state.comments.length; i++) {
				...
				replylink.innerText = isLogin ? 'Reply' : 'Signin to Reply'
	...
	renderEditor(state, instance) {
		...
		$container.append('<h3 class="comment-reply-title"><i class="fa fa-pencil"></i> We welcome your Comments and Suggestions</h3>')
		if (state.user.login) {
			...
			$container.append(`<div class="gitment-editor-write-field">
													<textarea id="comment" placeholder="Quickly post your comment!" cols="45" rows="7"></textarea>
												 </div>`)
			$container.append(`<p class="form-submit">
													<a class="gitment-editor-submit">Submit</a>
												 </p>`)
			...
			submitButton.onclick = function () {
				if(!submitButton.hasAttribute('disabled')) {
					submitButton.innerText = 'Submitting comments...'
					submitButton.setAttribute('disabled', true)
					instance.post(textarea.value.trim())
						.then(data => {
							textarea.value = ''
							submitButton.removeAttribute('disabled')
							submitButton.innerText = 'Comments'
						})
						.catch(error => {
							console.log(error)
							submitButton.removeAttribute('disabled')
			...
			$container.append(`<p class="must-log-in">
													To post a comment, you must first
